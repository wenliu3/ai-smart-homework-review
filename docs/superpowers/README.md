...
本目录只保存多智能体平台的架构与规格文档。业务说明继续放在项目根目录 `README.md`，执行历史以 Git 提交记录为准。
...
| 阶段 2：批改与查重工作流 | 已完成 | 生产批改已简化为 Celery 单模型多模态直调：作业要求、参考附件、学生正文、docx 内嵌图统一进消息，按规则模型单次评分（`temperature=0.2`），正则解析总分，失败保留原始回复并转人工；学生通过 gradingRunId 轮询进度。查重解释仍由助手审核后落 Run。 |
...
2026-07-27 阶段 3A/3B/4/5 后仓库级验证快照：
...
- Docker 必须使用 `docker compose --env-file .env.docker up -d --build`；仅 backend 执行双库迁移，worker 只启动 Celery，避免并发迁移。
- 自动化验证不替代真实三角色账号和外部模型/Redis 的端到端联调。

## 维护规则

- 规格发生架构变化时更新 `specs/`。
- 不在仓库中长期保存实施计划；任务执行过程由任务记录管理，最终结果写入规格、README 或 Git 历史。
