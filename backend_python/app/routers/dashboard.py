...
按角色隔离：/admin/* 仅 superadmin，/teacher/* 仅 teacher，/student/* 仅 student。
"""
...
def admin_overview(current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """管理员看板概览 — 用户/班级/作业/提交总数 + 分布统计"""
    return ok(dashboard_crud.get_admin_overview(db))


@router.get("/admin/dashboard/ai-models")
def ai_model_stats(current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """AI 模型使用统计 — 各模型在线状态/余额/用量"""
    return ok(dashboard_crud.get_ai_model_stats(db))


@router.get("/admin/dashboard/recent-users")
def recent_users(request: Request, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """获取最近新增的用户列表"""
...
def health(current_user: User = Depends(require_roles("superadmin"))):
    """系统健康检查（运维接口）— 运行时长/时间戳"""
    return ok(dashboard_crud.get_health())


# ========== 教师（仅教师）==========
@router.get("/teacher/dashboard/stats")
def teacher_stats(current_user: User = Depends(require_roles("teacher")), db: Session = Depends(get_db)):
...
def teacher_pending_tasks(current_user: User = Depends(require_roles("teacher")), db: Session = Depends(get_db)):
    """教师待办 — 即将截止作业 + 待批改提交"""
...
def student_stats(current_user: User = Depends(require_roles("student")), db: Session = Depends(get_db)):
