...
菜单/角色管理属系统管理功能，仅超级管理员可操作；
用户资源查询仅允许查询本人（superadmin 可查任意用户）。
"""
...
from ..schemas.permission import MenuCreate, MenuUpdate, RoleCreate, RoleUpdate
...
def get_user_resources(user_id: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    """获取用户资源 — 角色/权限/菜单树(user_id=current 表示当前用户；仅可查自己，superadmin 可查任意)"""
    return ok(permission_crud.get_user_resources(db, user_id, current_user))


# ===== 菜单管理（仅超级管理员）=====
@router.get("/permissions/menus")
def get_menus(request: Request, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
...
def get_menu_by_id(menu_id: int, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """根据 ID 查询菜单"""
    return ok(permission_crud.get_menu_by_id(db, menu_id))


@router.post("/permissions/menus")
def create_menu(body: MenuCreate, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """创建菜单 — 校验 code 唯一性"""
    return ok(permission_crud.create_menu(db, body.model_dump()))


@router.put("/permissions/menus/{menu_id}")
def update_menu(menu_id: int, body: MenuUpdate, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """更新菜单信息"""
...
def delete_menu(menu_id: int, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """删除菜单"""
...
def get_roles(request: Request, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """分页查询角色列表 — 支持搜索/状态过滤"""
    return ok(permission_crud.get_roles(db, dict(request.query_params)))


@router.get("/permissions/roles/{role_id}/with-menus")
def get_role_with_menus(role_id: int, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """查询角色及其关联的菜单"""
    return ok(permission_crud.get_role_with_menus(db, role_id))


@router.post("/permissions/roles")
def create_role(body: RoleCreate, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """创建角色 — 校验 code 唯一性"""
    return ok(permission_crud.create_role(db, body.model_dump()))


@router.put("/permissions/roles/{role_id}")
def update_role(role_id: int, body: RoleUpdate, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """更新角色信息"""
...
def delete_role(role_id: int, current_user: User = Depends(require_roles("superadmin")), db: Session = Depends(get_db)):
    """删除角色 — 系统内置角色不可删"""
